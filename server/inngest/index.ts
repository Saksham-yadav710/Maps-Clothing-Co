...
type MonthlyOffersData = {
  deals: Product[];
  users: Array<Pick<User, "name" | "email">>;
};
...
//monthly offer email(1st of every month promotion)
const sendMonthlyOffers = inngest.createFunction(
  ...
  async ({ step }) => {
    const { deals, users } = await step.run(
      ...
      async (): Promise<MonthlyOffersData> => {
    ...
    let sentCount = 0;
    //send in batches of 10 to not to overwhelm server
    const batchSize = 10;
      ...
      await step.run(`send-offers-batch-${i}`, async () => {
        for (const u of batch) {
          await sendEmail({
            ...
            subject: "Fresh picks from us - just for you",
...
//auto-assign riders every 5 minutes
  ...
  async ({ event, step }) => {
    ...
    const result = await step.run("assign-rider", async () => {
      ...
      if (order.deliveryPartnerId)
        return {
          skipped: true,
          reason: "Already Assigned the Driver",
        };
      ...
      const busyRiderIds = busyOrders.map(
        (o: { deliveryPartnerId: string | null }) => o.deliveryPartnerId,
      );
